#!/bin/bash
# Inference script for NTcouple solid/fuel field with GenCP framework (SiT_FNO backbone)
...
CHECKPOINT_PATH="/path/to/results/ntcouple/solid_SiT_FNO/ckpt/best_main_model.pth"

# Evaluate on couple_val (coupled evaluation split)
python infer_single_ntcouple.py \
  --config configs/ntcouple/solid_sit_fno.yaml \
