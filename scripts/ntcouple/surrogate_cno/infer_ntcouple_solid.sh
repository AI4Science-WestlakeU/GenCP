#!/bin/bash
# Inference script for NTcouple solid/fuel field with surrogate model
...
CHECKPOINT_PATH=/path/to/results/ntcouple/solid_CNO_surrogate/checkpoint/best_main_model.pth

# Evaluate on couple_val (coupled evaluation split)
python infer_single_ntcouple.py \
  --config configs/ntcouple/solid_surrogate_cno.yaml \
