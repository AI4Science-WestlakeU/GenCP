...
def setup_logging(experiment_name="analytical_experiment", log_dir="/path/to/plot_toy"):
    """
...
class FlowMatchingNetX(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=256):
        super(FlowMatchingNetX, self).__init__()
    ...
    def forward(self, x, t):
        ...
        if t.dim() == 0:
            t = t.unsqueeze(0)
        t_expanded = t.unsqueeze(-1).expand(x.shape[0], -1)
        input_with_time = torch.cat([x, t_expanded], dim=-1)
        return self.network(input_with_time).squeeze(-1)

class FlowMatchingNetY(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=256):
        super(FlowMatchingNetY, self).__init__()
        self.network = nn.Sequential(
            nn.Linear(input_dim + 1, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
...
def get_analytical_flow_matching_loss_x(model_x, mixed_joint_data, device):
    """Train vtx using analytically constructed mixed joint distribution p'(x,y)_x"""
    batch_size = mixed_joint_data.shape[0]
    
    t = torch.rand(batch_size, device=device)
    
    x0 = torch.randn_like(mixed_joint_data)
    x1 = mixed_joint_data
    ...
    v_pred_x = model_x(x_t, t)
    
    loss = nn.MSELoss()(v_pred_x, v_true_x)
    return loss

def get_analytical_flow_matching_loss_y(model_y, mixed_joint_data, device):
    """Train vty using analytically constructed mixed joint distribution p'(x,y)_y"""
    batch_size = mixed_joint_data.shape[0]
    ...
    v_pred_y = model_y(x_t, t)
    
    loss = nn.MSELoss()(v_pred_y, v_true_y)
    return loss

def train_analytical_flow_matching(model_x, model_y, mixed_joint_x_data, mixed_joint_y_data, 
                                 num_epochs=1000, batch_size=256, lr=1e-3, device='cpu'):
    """Train Flow Matching models for x and y components separately using analytically constructed mixed joint distributions"""
    model_x = model_x.to(device)
    model_y = model_y.to(device)
    
    mixed_joint_x_data = mixed_joint_x_data.to(device).float()
    mixed_joint_y_data = mixed_joint_y_data.to(device).float()
    
    optimizer_x = optim.Adam(model_x.parameters(), lr=lr)
    optimizer_y = optim.Adam(model_y.parameters(), lr=lr)
    
    dataset_x = TensorDataset(mixed_joint_x_data)
    dataloader_x = DataLoader(dataset_x, batch_size=batch_size, shuffle=True)
    
    dataset_y = TensorDataset(mixed_joint_y_data)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader_x:
            ...
            loss_x = get_analytical_flow_matching_loss_x(model_x, batch_data, device)
            loss_x.backward()
            optimizer_x.step()
        ...
        for batch in dataloader_y:
            ...
            loss_y = get_analytical_flow_matching_loss_y(model_y, batch_data, device)
            loss_y.backward()
            optimizer_y.step()
            epoch_loss_y += loss_y.item()
            num_batches_y += 1
        
        avg_loss_x = epoch_loss_x / num_batches_x if num_batches_x > 0 else 0
        avg_loss_y = epoch_loss_y / num_batches_y if num_batches_y > 0 else 0
        
        losses_x.append(avg_loss_x)
        losses_y.append(avg_loss_y)
        
        if epoch % 1 == 0:
            print(f"Epoch {epoch}, Loss X: {avg_loss_x:.6f}, Loss Y: {avg_loss_y:.6f}")
    
    return losses_x, losses_y


def sample_from_flow_matching_separate(model_x, model_y, mix_ratio=0.3, num_samples=1000, num_steps=100, device='cpu', visualize=False, target_name="2D Distribution"):
    model_x = model_x.to(device)
    model_y = model_y.to(device)
    model_x.eval()
    model_y.eval()
    ...
    if visualize:
        intermediate_states = []
        intermediate_states.append(x.cpu().numpy())
    with torch.no_grad():
        for step in range(num_steps):
            t = torch.full((num_samples,), step * dt, device=device)
            x_init = x.clone()
            
            # v_x = model_x(x_init, t)
            # x[:, 0] = x[:, 0] + dt * v_x
            # v_y = model_y(x_init, t)
            # x[:, 1] = x[:, 1] + dt * v_y
            
            # v_x = model_x(x_init, t)
            # x[:, 0] = x[:, 0] + dt * v_x
            # # print(f"x[:, 0]: {x[:, 0].shape}, x_init[:, 1]: {x_init[:, 1].shape}")
            # model_y_input = torch.cat([x[:, :1].clone(), x_init[:, -1:].clone()], dim=-1)
            # v_y = model_y(model_y_input, t)
            # x[:, 1] = x[:, 1] + dt * v_y
            ...
            v_y = model_y(model_y_input, t)
            x[:, 1] = x[:, 1] + dt * v_y    
            model_x_input = torch.cat([x[:, :1].clone(), x[:, -1:].clone()], dim=-1)
            v_x = model_x(model_x_input, t)
            x[:, 0] = x[:, 0] + dt * v_x // 2
            
            if visualize and step % (num_steps // 10) == 0:
                intermediate_states.append(x.cpu().numpy())
    if visualize:
        intermediate_states.append(x.cpu().numpy())
...
def save_distribution_plots(original_samples, generated_samples, target_name, mix_ratio):
    ...
    ax2.scatter(generated_samples[:, 0], generated_samples[:, 1], 
               alpha=0.6, s=1, color=COLORS['info'])
    ...
    # fig3.patch.set_facecolor(COLORS['light'])
    
    # ax3.scatter(generated_samples[:, 0], generated_samples[:, 1], 
    #            alpha=0.6, s=1, color=COLORS['secondary'])
    # # ax2.set_title(f'Generated Distribution\n({target_name})', 
    # #               fontsize=14, fontweight='bold', color=COLORS['dark'])
    # ax3.set_xlabel('X', fontsize=12, color=COLORS['dark'])
    # ax3.set_ylabel('Y', fontsize=12, color=COLORS['dark'])
    # ax3.set_xlim(x_lim)
    # ax3.set_ylim(y_lim)
    # ax3.grid(True, alpha=0.3, linestyle='--')
    # ax3.set_facecolor(COLORS['light'])
    ...
    # plt.tight_layout()
    # filename3 = f"{m2pde_dir}/generated_distribution_{target_name.lower().replace(' ', '_')}_mix_{mix_ratio}.png"
    # plt.savefig(filename3, dpi=300, bbox_inches='tight', facecolor=COLORS['light'])
    # plt.close(fig3)
    
    print(f"✅ Original distribution plot saved as: {filename1}")
    print(f"✅ Generated distribution plot saved as: {filename2}")
    # print(f"✅ Generated distribution plot saved as: {filename3}")
...
def run_analytical_experiment(mu_x=0.0, sigma_x=1.0, mu_y=0.0, sigma_y=1.0, rho=0.7, mix_ratio=0.3, complexity_level='simple'):
    """
    Run complete analytical distribution experiment
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    experiment_name = f"analytical_{complexity_level}_mix_{mix_ratio}"
    logger, log_filename = setup_logging(experiment_name)
    
    print(f"\n🔬 Starting Analytical Distribution Experiment")
    print(f"Parameters: μ_x={mu_x}, σ_x={sigma_x}, μ_y={mu_y}, σ_y={sigma_y}, ρ={rho}, mix_ratio={mix_ratio}")
    print("=" * 80)
    
    logger.info(f"Starting Analytical Distribution Experiment")
    ...
    # 1. Construct analytical joint distribution
    ...
    # 5. Train Flow Matching models
    print("\n🚀 Training Flow Matching models...")
    model_x = FlowMatchingNetX(input_dim=2, hidden_dim=256)
    model_y = FlowMatchingNetY(input_dim=2, hidden_dim=256)
    
    losses_x, losses_y = train_analytical_flow_matching(
        model_x, model_y, mixed_joint_x_samples, mixed_joint_y_samples,
        num_epochs=30, device=device
    )
    
    # 6. Sample from Gaussian noise to verify
    print("\n🎬 Sampling from Gaussian noise and verifying...")
    generated_samples = sample_from_flow_matching_separate(
        model_x, model_y, mix_ratio=mix_ratio, num_samples=10000, device=device, 
        visualize=True, target_name="GenCP Experiment"
    )
    print("\n📊 Computing distribution similarity metrics...")
    ...
    save_distribution_plots(original_joint_samples, generated_samples, "Analytical Experiment", mix_ratio)
    
    print("\n📊 Visualizing Flow Matching results...")
    visualize_2d_flow_matching_results(
        original_joint_samples, generated_samples, "Analytical Experiment", 
        losses_x, original_joint_samples
    )
    
    return (analytical_joint, mixed_marginal, model_x, model_y, 
            original_joint_samples, mixed_joint_x_samples, mixed_joint_y_samples, generated_samples)

if __name__ == "__main__":
    print("🎨 Analytical Distribution Flow Matching Experiment: Reconstructing Joint Distribution from Conditional Probabilities")
    print("=" * 80)
    
    print("\n🔬 Running Analytical Distribution Experiment...")
    print("=" * 50)
    ...
    # Run analytical distribution experiment
    for mix_ratio in [0.1]:
        results = run_analytical_experiment(
            mu_x=0.0, sigma_x=1.0, 
            mu_y=0.0, sigma_y=1.5, 
            rho=0.7, mix_ratio=mix_ratio,
    ...
    print("\n✅ Analytical Distribution Experiment Completed!")
    print("\nExperiment Summary:")
    print("- Constructed joint distribution p(x,y) with analytical expressions")
    print("- Calculated corresponding marginal distributions p(x), p(y) and conditional probabilities p(x|y), p(y|x)")
    print("- Constructed mixed marginal distributions p'(x), p'(y)")
    print("- Constructed mixed joint distributions p'(x,y)_x, p'(x,y)_y based on conditional probabilities")
    print("- Trained vtx and vty models separately")
    print("- Sampled from Gaussian noise to verify reconstruction of original joint distribution")