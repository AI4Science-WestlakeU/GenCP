...
class SiT(nn.Module):
    ...
    def forward(self, xt, t, x0, cond):
        if self.forward_type == 'SiT': # sit
            x = self.forward_SiT(xt, t, x0, cond)
            return x
        else: # latte
            x = self.forward_latte(xt, t, x0, cond)

            # # pdb.set_trace()
            # x_hard = (torch.sigmoid(x) > 0.5).float()

            # if self.training:
            #     x_soft = torch.sigmoid(x)
...
def SiT_S_2(**kwargs):
    return SiT(input_size=(64, 128), depth=12, hidden_size=384, patch_size=(2, 4), num_heads=6, **kwargs)
